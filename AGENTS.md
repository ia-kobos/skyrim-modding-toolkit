...
This is the canonical project instruction set for Codex and other compatible coding agents working in this repository.
...
runs a **localhost REST + MCP server inside the running game**, so Codex can inspect live state, run
...
Guards against the silent re-mastering / dropped-reference corruption class during bulk ESP operations (mod splits, plugin renames, master-list edits, YAML find-replace). Tool-agnostic; snapshots are stored outside the game directory.
...
This prevents accidental ESP corruption. Project rules forbid direct ESP writes, but xelib operates through Bash and can write via `SaveFile()`.

## Spriggit ESP Workflow (Preferred for Editing)

For **creating or editing ESP records**, prefer Spriggit over xelib. Spriggit serializes ESP files to human-readable YAML that Codex can edit directly — no FFI, no scripting layer, and the YAML diffs cleanly in git.
...
These are mandatory project rules. They are not automatic filesystem protections, so apply them as explicit decision gates before running a write operation.
...
After every session, near-miss, or unexpected outcome, evaluate whether a new rule, deterministic check, or knowledgebase entry could have prevented or caught the issue. Propose the smallest reliable protection when a pattern of risk emerges. Document proposed checks in the "Hook Candidates" section of `KNOWLEDGEBASE.md`.

### Backup discipline
- No automatic backup or audit trail is provided. Create and verify a backup before editing live game or configuration files.
- Record what changed and where the backup was placed in the task summary.
...
1. **Before any experimental change**, copy the current `.psc` source files to a user-approved backup location with a descriptive folder name (e.g. `known-good-pre-stagger/`). This is the rollback point.
2. **Back up scripts explicitly.** Spriggit auto-backups capture the ESP on every deserialize, but not `.psc`/`.pex` files.
...
- [ ] Evaluated whether this task reveals a gap in current safety coverage
